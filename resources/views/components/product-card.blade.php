@props(['product', 'compact' => false, 'showCategory' => true, 'showAddButton' => true])
...
<article
    {{ $attributes->merge([
        'class' =>
            'group bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl shadow-soft hover:shadow-lg transition flex flex-col',
    ]) }}>
    <div
        class="{{ $compact ? 'h-32' : 'h-40' }} bg-slate-50 dark:bg-slate-700 flex items-center justify-center rounded-t-2xl overflow-hidden">
        @if ($image)
